...
@section('content')
    ...
    <flux:text class="mt-2 mb-6 text-base">Efficient management of ticketing and travel orders.</flux:text>
                ...
                <flux:table.rows>
                    @forelse($tickets as $ticket)
                        <flux:table.row>
                            ...
                            <flux:table.cell>
                                <flux:button href="{{ route('admin.booking.show', $ticket->id) }}" icon="eye"
                                    size="xs" variant="filled">Full Details</flux:button>
                                @if (!$ticket->client_id)
                                    <flux:modal.trigger name="delete-ticket-{{ $ticket->id }}">
                                        <flux:button icon="trash" size="xs" variant="subtle" color="red" />
                                    </flux:modal.trigger>
                                        ...
                                        <div class="space-y-6">
                                            <div>
                                                ...
                                                <flux:text class="mt-2">
                                                    @if (!$ticket->client_id)
                                                        This booking has no owner. Would you like to remove it from the
                                                        system?
                                                    @else
                                                        You created this booking. Are you sure you want to delete it?
                                                    @endif
                                                </flux:text>
                                            </div>
                                            ...
                                            <div class="flex gap-2">
                                                ...
                                                <form action="{{ route('admin.booking.destroy', $ticket->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
