    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <flux:card>
                <div class="flex justify-between items-start mb-6">
                    ...
                    @php
                    $statusColor = match($tripTicket->status) {
                    'Pending' => 'yellow',
                    'Approved' => 'emerald',
                    'Cancelled' => 'red',
                    'Completed' => 'blue',
                    ...
                    @endphp
                    <flux:badge color="{{ $statusColor }}" inset="top bottom">{{ $tripTicket->status }}</flux:badge>
                </div>
            ...
            <flux:card>
                <div class="flex items-center justify-between mb-6">
                    ...
                    @else
                    <flux:badge size="sm" color="yellow" variant="subtle" icon="exclamation-circle">Pending Assignment
                    </flux:badge>
                    @endif
                </div>

                <div class="mb-8 p-4 rounded-xl bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10">
                    <form action="{{ route('trips.update', $tripTicket->id) }}" method="POST"
                        ...
                        <div class="flex-1">
                            ...
                            <flux:select name="driver_id" searchable placeholder="Select driver..." size="sm">
                                ...
                                <flux:select.option value="{{ $driver->id }}"
                                    :selected="$tripTicket->driver_id == $driver->id">
                                    {{ $driver->name }} — {{ $driver->vehicle->vehicle ?? 'No Vehicle' }}
                                </flux:select.option>
                                @endforeach
                            </flux:select>
                        </div>
                ...
                <div class="flex items-stretch gap-6">
                    <div class="flex-1">
                        <flux:text size="xs" class="uppercase tracking-wider font-semibold text-zinc-500 mb-2">Current
                            Driver</flux:text>
        ...
        <div class="space-y-6">
            ...
            <flux:card>
                ...
                <form action="{{ route('trips.update', $tripTicket->id) }}" method="POST" class="flex flex-col gap-2">
                    @csrf
                    @method('PUT')
