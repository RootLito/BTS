    ...
    <div class="w-full h-full flex flex-col" {{-- x-data="{
        ...
        initCalendar() {
            let calendar = new FullCalendar(this.$refs.calendar, {
                ...
                showNonCurrentDates: false,
                fixedWeekCount: false,
                events: this.events, // Adds the existing trips
                dayMaxEvents: 2,
                headerToolbar: {
                    left: 'title',
                    center: '',
                ...
                // Handle clicking existing trips
                eventClick: (info) => {
                    this.activeEvent = {
                ...
                // Handle selecting new dates for booking
                select: (info) => {
                    this.selection.start = info.start;
                    this.selection.end = info.end;
                    this.selection.displayStart = info.startStr;
        ...
        <div class="flex-1 grid grid-cols-3 gap-10 min-h-0">
            ...
            <div class="col-span-1 flex flex-col gap-10">
                ...
                <flux:card class="flex-1 flex flex-col">
                    <flux:heading size="lg" class="flex justify-between items-start">
                        Available Driver
                    </flux:heading>

                    <div class="flex items-end gap-1 mt-auto mb-4">
                        <flux:text class="text-3xl leading-none">
                    ...
                    <flux:modal.trigger name="driver-availability-modal">
                        <flux:button variant="primary" color="emerald" class="w-full mt-auto">
                            Check Availability
                        </flux:button>
                    </flux:modal.trigger>
                </flux:card>
                ...
                <flux:modal name="driver-availability-modal" class="md:w-[700px]">
                    <div class="space-y-6">
                        <div>
                            <flux:heading size="lg">Available Driver(s)</flux:heading>
                            <flux:subheading>Drivers available for assignment to a new trip ticket</flux:subheading>
                        </div>
                        ...
                        <flux:table>
                            <flux:table.columns>
                                ...
                                <flux:table.column>Current Destination</flux:table.column>
                            </flux:table.columns>
                                    ...
                                    <flux:table.row>
                                        ...
                                        <flux:table.cell>
                                            <span class="text-sm text-gray-500">
                                                {{ $driver->latestTrip->destination ?? 'No Recent Destination' }}
                                            </span>
                                        </flux:table.cell>
                                    </flux:table.row>
                                @endforeach
