                ...
                <div class="text-[11pt] leading-none space-y-4">
                    ...
                    <div class="grid grid-cols-12 gap-y-2">
                        ...
                        <div class="col-span-5 flex flex-col space-y-1 mt-2">
                            @foreach ($travelOrder->personnel ?? [] as $p)
                                <span
                                    class="border-b border-black uppercase min-h-[1.2rem]">{{ $p['position'] ?? '' }}</span>
                            @endforeach
                        </div>
                    ...
                    <div class="details-footer space-y-4 pt-6">
                        <div class="grid grid-cols-12 gap-y-2">
                            ...
                            <div class="col-span-4 border-b border-black uppercase">{{ $travelOrder->departure }}</div>
                            <div class="col-span-2 font-bold pl-4">Return Date:</div>
                            <div class="col-span-4 border-b border-black text-center uppercase">{{ $travelOrder->return }}
                            </div>
                        </div>

                        <div class="flex items-end">
                            <span class="font-bold w-[16.66%]">Destination:</span>
                            <span class="flex-1 border-b border-black uppercase">{{ $travelOrder->destination }}</span>
                        </div>
                        ...
                        <div class="space-y-2">
                            @php
                                $fields = [
                                    'Objective(s):' => $travelOrder->obj ?? '',
                                    'Per Diems Allowed:' => $travelOrder->per_diem ?? '',
                                    'Assistant Laborers Allowed:' => $travelOrder->laborers ?? '',
                                    'Appropriation to which travel should be charged:' =>
                                        $travelOrder->appropriation ?? '',
                                    'Remarks or Special Instructions:' => $travelOrder->remarks ?? '',
                                ];
                            @endphp

                            @foreach ($fields as $label => $value)
                                <div class="flex items-end">
                                    <span class="font-bold whitespace-nowrap mr-2">{{ $label }}</span>
                                    <span class="flex-1 border-b border-black h-5 uppercase">{{ $value }}</span>
                                </div>
                            @endforeach
                        </div>
    ...
    <flux:modal name="travel-order-info" class="w-300">
        <form action="{{ route('client.travel-order.store', $tripTicket->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <flux:text class="mt-2">Enter the details for this order. Fields will be blank for new entries.
                </flux:text>
            </div>
            ...
            <flux:textarea label="Purpose" name="purpose" placeholder="Specific purpose of trip..." rows="2"
                class="mb-4">
            </flux:textarea>
